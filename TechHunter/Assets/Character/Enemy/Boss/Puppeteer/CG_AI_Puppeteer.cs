...
public class CG_AI_Puppeteer : Boss_MoveBase
{
    ...
    public float targetDistance = 2f;   // 最終的に保ちたい距離
    public float approachSpeed = 2f;    // 距離を近づくスピード
    ...
    void SpawnEnemy() 
    {
        for (int i = 0; i < 2; i++)
        {
            ...
            GameObject CL_Enemy0 = EnemySpawn(Enemys[Random.Range(0, Enemys.Count)], newPos);
        ...
        GameObject CL_Enemy1 = EnemySpawn(Enemys[1], newPos2);
    ...
    void NeedSpawnEnemy(GameObject ZakoEnemy,Vector2 ZakoPos)
    {
         ...
         GameObject CL_Enemy = EnemySpawn(ZakoEnemy, ZakoPos);
    ...
    void SpawnZakoEnemy()
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            GameObject CL_Enemy = EnemySpawn(Zako, newPos);
            EnemyContoroller enemyContoroller1 = CL_Enemy.GetComponent<EnemyContoroller>();
            enemyList.Add(enemyContoroller1);
            Instantiate(DieEffect, newPos, Quaternion.identity);
    ...
    void A6() 
    {
        ...
        for (int i = 0; i < 6; i++) 
        {
            ...
            GameObject CL_Enemy = EnemySpawn(Enemys[Random.Range(0,Enemys.Count)],newPos);
            EnemyContoroller enemyContoroller1 = CL_Enemy.GetComponent<EnemyContoroller>();
            enemyList.Add(enemyContoroller1);
    ...
    void A7()
    {
        ...
        if (enemyBase.mode == EnemyBase.ModeType.M15)
        {
            ...
            for (int i = 0; i < 30; i++) 
            {
                StartCoroutine(DereySpawnEnemy(0.4f * i,Zako));

                if (i % 5 == 0) 
                {
                    StartCoroutine(DereySpawnEnemy(0.4f * i, Zako1));
                }
                if (i % 10 == 0) 
                {
                    StartCoroutine(DereySpawnEnemy(0.4f * i, Enemys[0]));
                }
            }
            enemyBase.mode = EnemyBase.ModeType.M2;
            enemyBase.AttackCount = 0; AttackNumber = 0;
        ...
        if (enemyBase.mode == EnemyBase.ModeType.M2)
        {
            ...
            RotateMove();
            sabAttackCount += Time.fixedDeltaTime;
            
            if (sabAttackCount > 0.4f) 
            {AllEnemy.RemoveAll(e => e == null);
                for (int i = 0; i < 2; i++) 
                {
                    Vector2 newPosition = (Vector2)transform.position + Vector2.one * Random.Range(-1f, 1f);
                    NeedleSpawn(newPosition);
                }
                sabAttackCount = 0;
            }
            
            if (enemyBase.AttackCount > 35 || AllEnemy.Count == 0)
            {
    ...
    private IEnumerator DereySpawnEnemy(float delayTime, GameObject zakoEnemy)
    {
        ...
        Vector2 newPos = EnemySpawnLists_GameObject.gameObjects[Random.Range(0, EnemySpawnLists_GameObject.gameObjects.Count)].transform.position;
        GameObject CL_Enemy1 = EnemySpawn(zakoEnemy, newPos);

    }
    [SerializeField] Vector2 centerPos = Vector2.zero;

    float rotateangle = 0f;
    bool initializedAngle = false;
    void RotateMove()
    {
        Vector2 pos = enemyBase.transform.position;
        Vector2 toCenter = -pos;
        ...
        // --- 目標半径に近づく（ワープ防止） ---
        float newDist = Mathf.MoveTowards(dist, targetDistance, approachSpeed * Time.deltaTime);

        // --- 新しい位置を計算 ---
        float x = Mathf.Cos(angle) * newDist;
        float y = Mathf.Sin(angle) * newDist;

        enemyBase.transform.position = new Vector2(x, y);
    }
}
