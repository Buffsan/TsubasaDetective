...
public class CG_CaptainKnight_AI : EnemyMoveBase
{
    [SerializeField] GameObject Arrow;
    [SerializeField] EnemyContoroller enemyContoroller;
    [SerializeField] GameObject CG_Spear;
    [SerializeField] GameObject CG_Knight;
    int Phase = 0;
    ...
    public override void EnemyAttackPlay()
    {
        ...
        if (enemyBase.mode == EnemyBase.ModeType.M5)
        {
            ...
            for(int i =0; i < 1;i++)
            SpawnEnemySpawn(CG_Spear, transform.position, Quaternion.Euler(0, 0, 0), 3);
            for (int i = 0; i < 8; i++)
                SpawnEnemySpawn(CG_Knight, transform.position, Quaternion.Euler(0, 0, 0), 5);
