...
public class sytem_GameSpot : MonoBehaviour
{
    ...
    [SerializeField] GameObject MapExplain;
    ...
    void Update()
    {
        if (Input.GetMouseButtonDown(0)&& Totch) 
        {
            ...
            else 
            {
                ...
                GameObject CL_MapExplain = Instantiate(MapExplain,new Vector2(3000,3000),Quaternion.identity);
                CL_MapExplain.transform.parent = controller.MapExplainObject.transform;
                SpotBoard_Controller spotBoard_Controller = CL_MapExplain.GetComponent<SpotBoard_Controller>();
                spotBoard_Controller.gameSpot = this;
                if (ALL_SystemManager.Instance.sytem_GameSpotsController.PlayerSystemPos.x < 2) {
                    spotBoard_Controller.ChangeUI(SpotInfo,1);
                }
                else 
                {
                    spotBoard_Controller.ChangeUI(SpotInfo, -1);
