...
public class sytem_GameSpotsController : MonoBehaviour
{
    ...
    [SerializeField] GameObject spotBackGround;
    public Vector2 PlayerSystemPos = Vector2.zero;
    public GameObject MapExplainObject;
    public GameObject MapExplainPrefab;
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.K) && systemmanager.gameSystemMode == SystemManager.GameSystemMODE.Debug)
        {
    ...
    public void isLastNextSpotStart() 
    {
        systemmanager.NextStageReset();
        gamemode.allgamemode = AllGameMode.SpotChoiceMode;
        DestroyInputSpots();
    ...
    void UpDateLastInputSpot() 
    {
        ...
        for (int f = 0; f < 16; f++)
        {
            ...
            {
                GameSpot_SpotInfo clonedInfo = SpotInfos[6].Clone(); // DeepCloneメソッドを仮定
                InputSpot.Add(clonedInfo); // クローンを追加
            }
        }
    }
    public void UpdateInputSpot() 
    {
        ...
        for (int f = 0; f < 16; f++)
        {
            ...
            float NextSpawRate = 0; 
            float RandomRate = Random.Range(0, (int)AllSpawRate);
            foreach (GameSpot_SpotInfo spInfo in SpotInfos)
            {   
    ...
    public void RandomCloneSpot(float X,float Y,int Number) 
    {
        ...
        if (CL_gameSpot.SpotInfo.Name == SpotInfos[6].Name) 
        {
            CL_Spot.SetActive(false);
